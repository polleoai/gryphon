...
const MODEL_ALIAS = (() => {
    ...
    for (const [alias, perVendor] of Object.entries(registry.CROSS_VENDOR_ALIASES)) {
        const pv = perVendor;
        if (pv.anthropic)
            out[alias] = pv.anthropic;
    }
    for (const m of registry.modelsByVendor("anthropic")) {
        if (out[m.id] && out[m.id] !== m.id) {
            throw new Error(`pricing/anthropic.js: alias collision — concrete id "${m.id}" would overwrite ` +
...
function computeCost(usage, modelId) {
    ...
    const inputTokens = _finite(usage.input_tokens) +
