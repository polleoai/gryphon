...
const MODEL_ALIAS: Record<string, string> = (() => {
  ...
  for (const [alias, perVendor] of Object.entries(registry.CROSS_VENDOR_ALIASES)) {
    ...
    if (pv.openai) out[alias] = pv.openai;
  }
...
function computeCost(usage: Record<string, unknown> | null | undefined, modelId: string) {
  ...
  const ptd = usage.prompt_tokens_details as Record<string, unknown> | null | undefined;
...
function getModelDropdownOptions() {
  return registry.dropdownFor("openai").map((o: { id: string; label: string; desc: string }) => ({
    id: o.id,
...
function getCodexCliModelDropdownOptions() {
  return getModelDropdownOptions().filter((opt: { id: string; label: string }) =>
 ...
 */
function coerceToCodexCliModel(alias: string | null | undefined): string {
