...
test("installHookDeadline: deadline timer fires fail-closed when stdin hangs", async () => {
  ...
  // ipc-client is now .ts — use --import tsx so node can load it.
  // The require path is extensionless so tsx resolves to .ts on disk.
  const ipcClientPath = path.join(__dirname, "..", "src", "hooks", "common", "ipc-client");
  const child = spawn(process.execPath, ["--import", "tsx", "-e", `
    const { installHookDeadline } = require(${JSON.stringify(ipcClientPath)});
