...
function _shortPathOf(dir) {
    try {
        ...
        const r = spawnSync("cmd", ["/c", `for %I in ("${dir}") do @echo %~sI`], { encoding: "utf8", timeout: 5000, windowsVerbatimArguments: true });
        if (!r || r.status !== 0)
            return null;
...
function _writeWinShim(shimPath, nodePath, scriptPath, ipcSocketPath) {
    ...
    try {
        ...
        console.warn(`[gryphon/antigravity-hooks] could not write the hook shim ${shimPath}: ` +
            `${e.message}`);
        return false;
    }
}
function _makeCommand(nodePath, scriptPath, ipcSocketPath) {
    const env = [
 ...
 */
function _buildHookEntry({ pluginDir, nodePath, ipcSocketPath = "" }) {
...
function _readHooks(file) {
    ...
    try {
        ...
        console.warn(`[gryphon/antigravity-hooks] ${file} is not valid JSON — leaving it untouched ` +
            `and running without hook enforcement (${e.message})`);
        return null;
    }
}
/**
 ...
 */
function _writeHooks(file, json) {
 ...
 */
function _installInto(file, { pluginDir, nodePath, ipcSocketPath = "" }) {
    ...
    if (!entry)
        return false; // unquotable path — degrade, never emit an injectable command
    json[HOOK_KEY] = entry;
    try {
        ...
        console.warn(`[gryphon/antigravity-hooks] failed to write ${file}: ${e.message}`);
        return false;
    }
}
/**
 ...
 */
function _uninstallFrom(file, ownedSocket) {
    ...
    const shim = json[HOOK_KEY]?.PreToolUse?.[0]?.hooks?.[0]?.command;
    ...
    try {
        ...
        console.warn(`[gryphon/antigravity-hooks] failed to clean ${file}: ${e.message}`);
        return false;
    }
}
/**
 ...
 */
function buildSpawnExtras({ pluginDir, ipcSocketPath, nodePath, _hooksFile }) {
    ...
    if (!_installInto(file, { pluginDir, nodePath, ipcSocketPath }))
        return null;
    let cleaned = false;
    const cleanup = () => {
        if (cleaned)
