...
function _ensureSessionCache(plugin) {
    if (!plugin)
        return null;
    if (!plugin._permSessionCache)
        plugin._permSessionCache = new Map();
 ...
 */
async function checkPermission({ ctx, action, target, detail, cacheable = true, kind = "fileEdit", warning = null, category = null, categoryTitle = null, }) {
    const mode = ctx.permissionMode || "default";
    const ctxPlugin = ctx.plugin;
    const settings = (ctxPlugin && ctxPlugin.settings) || {};
    const protectedModeOn = settings.protectedMode !== false; // default true
    const autoDenyProtected = settings.autoDenyProtected === true;
    if (mode === "plan") {
        return {
            allow: false,
            reason: `Plan mode is active — ${action} on ${target} is not permitted. ` +
    ...
    const cache = cacheable && !isProtected ? _ensureSessionCache(ctxPlugin) : null;
    ...
    const decisionTyped = decision;
    if (cache && decisionTyped.remember) {
        cache.set(target, decisionTyped.allow ? "always" : "deny-always");
    }
    ...
    return {
        allow: decisionTyped.allow,
        reason: decisionTyped.allow ? "" : refusalReason,
    };
}
...
function _showPermissionModal({ app, action, target, detail, showRememberToggle = true, warning = null, isProtected = false, category = null, categoryTitle = null, assistantName = "Claude", }) {
    ...
    return new Promise((resolve) => {
        ...
        modal.titleEl.setText(isProtected
        ...
        if (isProtected) {
            ...
            modal.contentEl.createEl("p", {
                ...
                text: `If you click Approve, ${assistantName} will go ahead with this operation. ` +
        ...
        if (isProtected) {
            buttons
                .addButton((btn) => btn.setButtonText("Approve").onClick(() => finish(true)))
                .addButton((btn) => btn.setButtonText("Deny").setCta().onClick(() => finish(false)));
        }
        else {
            buttons
                .addButton((btn) => btn.setButtonText("Deny").onClick(() => finish(false)))
                .addButton((btn) => btn.setButtonText("Allow").setCta().onClick(() => finish(true)));
        }
        // Keyboard shortcuts:
