            ...
            const finishListen = () => {
                if (!IS_WINDOWS) {
                    ...
                    try {
                        ...
                        chmodOk = false;
                        try {
                            console.warn("[gryphon/ipc] chmod failed:", e && e.message);
                        }
                        catch (_) { }
                    }
                    if (!chmodOk) {
                        try {
                            ...
                            if (worldBits !== 0) {
                                ...
                                this._socketPath = null;
                                reject(new Error(`IPC socket perms too permissive (mode ${st.mode.toString(8)}) ` +
                ...
                server.on("error", (e) => {
                    try {
                        console.warn("[gryphon/ipc] server error:", e.message);
                    }
                    catch (_) { }
