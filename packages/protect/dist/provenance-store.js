...
class ProvenanceStore {
    ...
    _reloadFromDisk() {
        ...
        try {
            ...
            this._loadError = e instanceof Error ? e : new Error(String(e));
            this._loaded = true;
            try {
                console.warn(`[gryphon/provenance] load failed (writes blocked until cleared): ${this._loadError.message}`);
            }
            catch (_) { }
            return false;
        }
        try {
            ...
            if (parsed && typeof parsed === "object" && parsed.paths && typeof parsed.paths === "object") {
                ...
                try {
                    console.warn(`[gryphon/provenance] ${this._loadError.message}`);
                }
                catch (_) { }
            }
        }
            ...
            this._loadError = e instanceof Error ? e : new Error(String(e));
            try {
                console.warn(`[gryphon/provenance] load failed (JSON parse): ${this._loadError.message}`);
            }
            catch (_) { }
        }
        this._loaded = true;
    ...
    add(vaultRelPath, metadata) {
        const key = _normaliseKey(vaultRelPath);
        if (!key)
            throw new Error("ProvenanceStore.add: empty path");
    ...
    _withLock(fn) {
            ...
            try {
                ...
                try {
                    ...
                    if (Date.now() - stat.mtimeMs > LOCK_STALE_MS) {
                        ...
                        catch (_) { /* another writer unlinked first */ }
                        continue; // try again immediately
                    }
                }
                catch (_) { /* lockfile vanished between EEXIST and stat → loop retries */ }
                if (Date.now() >= deadline) {
                    throw new Error(`ProvenanceStore: could not acquire lock within ${LOCK_TIMEOUT_MS}ms ` +
        ...
        try {
            ...
            catch (_) { /* ignore — stale-lock logic handles */ }
        }
    }
    ...
    _persist() {
        if (this._loadError) {
            ...
            throw new Error(`provenance store write blocked: prior load failed (${this._loadError.message}). ` +
