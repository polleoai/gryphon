...
class ProvenanceStore {
  ...
  _file: string;
  _lockFile: string;
  _data: { version: number; paths: Record<string, unknown> };
  _loaded: boolean;
  ...
  _reloadFromDisk() {
    ...
    try {
      ...
      this._loaded = true;
      try { console.warn(`[gryphon/provenance] load failed (writes blocked until cleared): ${this._loadError.message}`); } catch (_) {}
      return false;
    }
    ...
    try {
      ...
      // recover by hand.
      this._loadError = e instanceof Error ? e : new Error(String(e));
      try { console.warn(`[gryphon/provenance] load failed (JSON parse): ${this._loadError.message}`); } catch (_) {}
    }
    this._loaded = true;
   ...
   */
  has(vaultRelPath: string): boolean {
    if (!this._loaded) this.load();
   ...
   */
  get(vaultRelPath: string): unknown {
    if (!this._loaded) this.load();
    const key = _normaliseKey(vaultRelPath);
    if (!key) return null;
   ...
   */
  add(vaultRelPath: string, metadata: { source: string; sourceUrl?: string; sourceCommand?: string; taggedAt?: string; sessionId?: string }) {
    const key = _normaliseKey(vaultRelPath);
 ...
 */
function _normaliseKey(p: unknown): string | null {
