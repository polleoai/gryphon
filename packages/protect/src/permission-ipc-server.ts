...
class PermissionIPCServer {
  _server: import("net").Server | null;
  _socketPath: string | null;
  _handlers: Map<string, (req: Record<string, unknown>) => unknown>;
  _activeSockets: Set<import("net").Socket>;
  _creatingPromise: Promise<void> | null;
   ...
   */
  on(reqType: string, handler: (req: Record<string, unknown>) => unknown) {
   ...
   */
  async create(socketPath: string) {
  ...
  async _doCreate(socketPath: string) {
    ...
    await new Promise<void>((resolve, reject) => {
      const server: import("net").Server = net.createServer((socket: import("net").Socket) => this._onConnection(socket));
      ...
      const finishListen = () => {
        if (!IS_WINDOWS) {
          ...
          try { fs.chmodSync(socketPath, 0o600); }
            ...
            chmodOk = false;
            try { console.warn("[gryphon/ipc] chmod failed:", e && (e as Error).message); } catch (_) {}
          }
          if (!chmodOk) {
            try {
  ...
  _onConnection(socket: import("net").Socket) {
  ...
  async _handleLine(socket: import("net").Socket, line: string) {
    ...
    try {
      ...
      this._safeWrite(socket, { resp: "error", error: "invalid-json", detail: (e as Error).message });
      return;
    }
    ...
    const response: Record<string, unknown> = Object.assign({}, result || {}, { id: reqId });
    if (!response.resp) response.resp = reqType;
    this._safeWrite(socket, response);
  }

  _safeWrite(socket: import("net").Socket, obj: Record<string, unknown>) {
    try {
