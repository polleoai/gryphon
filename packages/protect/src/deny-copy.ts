 ...
 */
function describeOperation(action: string, target: unknown) {
 ...
 * file paths".
 */
function settingsPathForKind(kind: string): string {
 ...
 * @returns {string}
 */
function buildDenyReason({ action, target, category, kind }: { action: string; target: unknown; category: string | null | undefined; kind: string }) {
