...
function _tryUnlink(full: string): boolean {
...
function sweepHookSettingsOrphans({ tmpDir, cutoffMs = DEFAULT_SETTINGS_CUTOFF_MS }: { tmpDir?: string; cutoffMs?: number } = {}) {
...
function sweepSocketOrphans({ tmpDir }: { tmpDir?: string } = {}) {
...
function sweepProvenanceTmpOrphans({ pluginDir, cutoffMs = DEFAULT_PROVENANCE_TMP_CUTOFF_MS }: { pluginDir?: string; cutoffMs?: number } = {}) {
...
function sweepChatHistoryTmpOrphans({ pluginDir }: { pluginDir?: string } = {}) {
  if (!pluginDir) return { removed: [] };
  let entries;
  try { entries = fs.readdirSync(pluginDir); } catch (_) { return { removed: [] }; }
  const removed = [];
 ...
 */
function truncateHookTraceLog({ tmpDir }: { tmpDir?: string } = {}) {
  const dir = tmpDir || os.tmpdir();
 ...
 */
function sweepGryphonOrphans(opts: { pluginDir?: string; tmpDir?: string; truncateTraceLog?: boolean } = {}) {
  const summary: Record<string, unknown> & {
    hookSettings: { removed: string[] };
    sockets: { removed: string[] };
    provenanceTmp: { removed: string[] };
    chatHistoryTmp: { removed: string[] };
  } = {
