 ...
 * @returns {string}
 */
function buildFraming({ tool, sourceDetail, injectionHits }: { tool?: string; sourceDetail?: string; injectionHits?: unknown[] } = {}): string {
  const label = (tool && TOOL_LABELS[tool]) || `an external tool (${tool || "unknown"})`;
 ...
 */
function shouldFrame(tool: string): boolean {
  switch (tool) {
    case "WebFetch":
    case "WebSearch":
    case "Bash":
