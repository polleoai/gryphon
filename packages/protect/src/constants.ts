...
// Protect-domain data: protected-path catalog, protected-command catalog,
// and category metadata. Moved out of plugin/src/constants.js as part of
// the v1.5 three-axis split (Stage 2). Future engine/content split will
// JSON-ify these into packages/protect/content/*.json with schemaVersion.

import type { ProtectedPath, ProtectedCommand } from "./types";
...
const DEFAULT_PROTECTED_COMMANDS: ProtectedCommand[] = [
  {
  ...
  },
  {
    ...
    category: "runs-arbitrary-code",
    userRisk:
      "This opens a reverse shell — a live connection that hands control of your machine to a remote computer, letting whoever is on the other end run commands as you. It is almost never something a legitimate setup step needs to do.",
