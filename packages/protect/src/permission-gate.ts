...
function _ensureSessionCache(plugin: Record<string, unknown> | null | undefined): Map<string, string> | null {
  if (!plugin) return null;
  if (!plugin._permSessionCache) plugin._permSessionCache = new Map<string, string>();
  return plugin._permSessionCache as Map<string, string>;
}
...
async function checkPermission({
  ...
  const decisionTyped = decision as { allow: boolean; remember?: boolean };
...
function _clip(text: unknown, max: number): unknown {
 ...
 */
function _getAssistantName(ctx: Record<string, unknown>): string {
  try {
    ...
    if (!e || (e as NodeJS.ErrnoException).code !== "MODULE_NOT_FOUND") {
      console.error("[gryphon/permission-gate] _getAssistantName threw unexpectedly:", e && (e as Error).message);
    }
    return "Claude";
  }
}
  ...
  return new Promise((resolve) => {
    ...
    if (showRememberToggle) {
      new Setting(modal.contentEl)
        ...
        .addToggle((t: { setValue: (v: boolean) => typeof t; onChange: (cb: (v: boolean) => void) => typeof t }) => t.setValue(false).onChange((v: boolean) => { remember = v; }));
    }
    ...
    if (isProtected) {
      buttons
        .addButton((btn: { setButtonText: (s: string) => typeof btn; onClick: (cb: () => void) => typeof btn }) =>
          btn.setButtonText("Approve").onClick(() => finish(true))
        )
        .addButton((btn: { setButtonText: (s: string) => typeof btn; setCta: () => typeof btn; onClick: (cb: () => void) => typeof btn }) =>
