// TypeScript module marker — keeps global scope isolated from other .ts files.
...
class PathOutsideVaultError extends Error {
  requested: string;
  vaultRoot: string;
  constructor(requested: string, vaultRoot: string) {
 ...
 */
function resolveVaultPath(requested: string, vaultRoot: string): string {
...
function _isInside(child: string, parent: string): boolean {
 ...
 */
function matchProtectedPath(vaultRel: string, patterns: Array<string | { pattern: string }>): string | null {
 ...
 */
function parsePatternList(settingValue: unknown): string[] {
 ...
 * at tool-call time.
 */
function resolveActivePatterns(defaults: unknown, disabled: unknown, custom: unknown): string[] {
