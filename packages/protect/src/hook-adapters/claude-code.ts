 ...
 */
function buildSpawnExtras({ pluginDir, ipcSocketPath, nodePath, permissionsOnly }: { pluginDir: string; ipcSocketPath: string; nodePath: string; permissionsOnly?: { denyGlobs: string[] } | null }) {
  ...
  const cleanup = () => {
    ...
    try {
      ...
      console.warn(`[gryphon/hook-adapter] failed to remove ${settingsFile}: ${(e as Error).message}`);
    }
    settingsFile = null;
  };
