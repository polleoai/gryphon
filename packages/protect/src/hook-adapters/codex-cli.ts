 ...
 */
function _buildModelInstructions(): string {
  const bullets = GRYPHON_SYSTEM_PROMPT_HINT.split("· ").map((s: string) => s.trim()).filter(Boolean);
  const head = bullets[0];
  const rest = bullets.slice(1).map((b: string) => `- ${b}`).join("\n\n");
  return [
 ...
 */
function _buildHooksToml({ pluginDir, nodePath, modelInstructionsFile }: { pluginDir: string; nodePath: string; modelInstructionsFile: string }) {
 ...
 */
function _createCodexHomeOverlay({ pluginDir, nodePath }: { pluginDir: string; nodePath: string }): string {
  ...
  for (const name of PRESERVED_FROM_REAL_HOME) {
    ...
    try {
      ...
      if ((e as NodeJS.ErrnoException).code === "EPERM" && (name === "auth.json" || name === "session_index.jsonl")) {
        try { fs.copyFileSync(real, linkPath); }
        catch (e2) { console.warn(`[gryphon/codex-hooks] couldn't preserve ${name}: ${(e2 as Error).message}`); }
      } else if ((e as NodeJS.ErrnoException).code !== "EEXIST") {
        console.warn(`[gryphon/codex-hooks] symlink ${name} failed: ${(e as Error).message}`);
      }
    }
  }
 ...
 */
function _cleanupOverlay(overlay: string | null) {
  if (!overlay) return;
  try {
    ...
    console.warn(`[gryphon/codex-hooks] failed to remove overlay ${overlay}: ${(e as Error).message}`);
  }
}
