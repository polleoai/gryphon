...
function _writeSettingsFile(settings: Record<string, unknown>): string {
...
function _cleanupFile(file: string | null | undefined) {
  if (!file) return;
  try {
    if (fs.existsSync(file)) fs.unlinkSync(file);
  } catch (e) {
    console.warn(`[gryphon/gemini-hooks] failed to remove ${file}: ${(e as Error).message}`);
  }
}
