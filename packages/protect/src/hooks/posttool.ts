...
// about the SDK path.
let scanForInjectionMarkers: ((text: string) => Array<{ id: string; severity: string; match: string; offset: number }>) | undefined;
let buildFraming: ((opts: { tool?: string; sourceDetail?: string; injectionHits?: unknown[] }) => string) | undefined;
let shouldFrame: ((tool: string) => boolean) | undefined;
try {
...
async function main() {
  ...
  let sourceAttribution: string | null = null;  // overrides framing source if non-null
  try {
  ...
  const sourceDetail: string | undefined = (sourceAttribution || _summarizeSource(toolName as string, toolInput as Record<string, unknown> | null)) || undefined;

  let hits: Array<{ id: string; severity: string; match: string; offset: number }> = [];
  if (scannable && typeof scanForInjectionMarkers === "function") {
    try {
      ...
      console.error("[gryphon/posttool] scanForInjectionMarkers threw:", e && (e as Error).message);
    }
  }
 ...
 */
function _extractScannableText(response: unknown): string {
  if (typeof response === "string") return response;
  if (!response || typeof response !== "object") return "";
  const r = response as Record<string, unknown>;
  // Bash shape: { stdout, stderr, interrupted, isImage, noOutputExpected }
  if (typeof r.stdout === "string" || typeof r.stderr === "string") {
    return [r.stdout as string | undefined, r.stderr as string | undefined].filter(Boolean).join("\n");
  }
 ...
 */
function _summarizeSource(tool: string, input: Record<string, unknown> | null | undefined): string | null {
  if (!input) return null;
  if (tool === "WebFetch") return typeof input.url === "string" ? input.url : null;
  if (tool === "WebSearch") return typeof input.query === "string" ? `query: ${input.query}` : null;
  if (tool === "Bash") return typeof input.command === "string" ? `cmd: ${input.command}` : null;
  if (tool === "Read") return typeof input.file_path === "string" ? input.file_path : null;
  if (tool === "Glob") return typeof input.pattern === "string" ? input.pattern : null;
  if (tool === "Grep") return typeof input.pattern === "string" ? `pattern: ${input.pattern}` : null;
  return null;
}
 ...
 */
function _isOutsideVault(filePath: unknown, vaultRoot: unknown): boolean {
