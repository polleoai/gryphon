 ...
 */
async function sendToGryphon(request: any, options: any = {}) {
 ...
 */
async function emitEvent(type: string, fields: Record<string, unknown> = {}) {
  try {
    await sendToGryphon(
 ...
 */
function writeStdoutJson(obj: any) {
 ...
 */
function installHookDeadline({ deadlineMs, onTimeoutPayload, onCrashPayload }: { deadlineMs: number; onTimeoutPayload: unknown; onCrashPayload: unknown }) {
  if (typeof deadlineMs !== "number" || deadlineMs <= 0) {
    throw new TypeError("installHookDeadline: deadlineMs must be a positive number");
  }
  let emitted = false;
  async function emitAndExit(payload: unknown) {
    if (emitted) return;
    emitted = true;
    try {
 ...
 */
function traceHook(hookName: string, input: unknown) {
