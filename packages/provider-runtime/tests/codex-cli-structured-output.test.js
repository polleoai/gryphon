/**
 * Task 3.3 — CodexProvider structured output with L6.1 retry budget.
 *
 * Validates:
...
Module._load = function (request, ...args) {
  ...
  if (request === "@gryphon/provider-config") {
    const pricing = {
      openai: {
        computeCost: () => ({ cost: 0 }),
        coerceToVendorModel: (m) => m || "gpt-5.5",
...
test("codex-cli: structured output succeeds on first valid attempt", async () => {
  let callCount = 0;
...
test("codex-cli: retries up to 3 times on schema failure, then throws budget-exhausted", async () => {
...
test("codex-cli: onDone is restored after _spawnTurn rejects mid-retry (regression for null-onDone leak)", async () => {
...
test("codex-cli: no structuredOutput option → plain text passthrough", async () => {
