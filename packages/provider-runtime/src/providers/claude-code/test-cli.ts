...
function testCli(claudePath: any) {
  return new Promise((resolve) => {
    ...
    try {
      ...
      resolve({ ok: false, message: `Could not spawn Claude Code CLI: ${(err as Error).message}` });
      return;
    }
    ...
    timer = setTimeoutFn(
      () => finish({ ok: false, message: `Claude Code CLI timed out (${_LIVENESS_TIMEOUT_MS / 1000}s) waiting for a completion.` }),
