...
async function execute(input: any, ctx: any) {
  ...
  try {
    ...
    return _error(`Fetch failed: ${(e as Error).message}`);
  }
 ...
 */
async function _resolveAndCheck(hostname: any) {
  let addrs;
  try {
...
function _isPrivateHost(hostname: any) {
...
function _htmlToText(html: any) {
  return html
    ...
    .replace(/&gt;/gi, ">")
    .replace(/&quot;/gi, '"')
    .replace(/&#(\d+);/g, (_: any, code: any) => String.fromCharCode(parseInt(code, 10)))
    .replace(/[ \t]+/g, " ")
    .replace(/\n[ \t]+/g, "\n")
    .replace(/\n{3,}/g, "\n\n")
    .trim();
...
function _isAllowedContentType(contentType: any) {
