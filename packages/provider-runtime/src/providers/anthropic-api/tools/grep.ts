...
async function execute(input: any, ctx: any) {
  ...
  try {
    ...
    return _error(`Invalid regex pattern: ${(e as Error).message}`);
  }
  ...
  const results: Array<{ path: string; line: string; lineNo: number }> = [];  // { path, line, lineNo, count }
