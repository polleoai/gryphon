...
async function execute(input: any, ctx: any) {
  ...
  try {
    ...
    return _error(String((e as Error).message || e));
  }
  ...
  try {
    ...
    return _error(`Failed to read file: ${(e as Error).message}`);
  }
  ...
  try {
    ...
    return _error(`Failed to re-read file before write: ${(e as Error).message}`);
  }
  if (current !== original) {
    return _error(
  ...
  try {
    ...
    if ((e as NodeJS.ErrnoException).code === "ELOOP" || (e as NodeJS.ErrnoException).code === "EMLINK") {
      return _error(
        `Refusing to follow a symlink at the edit target: ${filePath}. ` +
        `The file was replaced with a symlink during the edit.`
      );
    }
    return _error(`Failed to write file: ${(e as Error).message}`);
  } finally {
...
function _countOccurrences(haystack: any, needle: any) {
  if (!needle) return 0;
  let count = 0;
  let pos = 0;
...
function _buildDiffPreview(oldString: any, newString: any) {
...
function _ok(text: any) {
  return { content: [{ type: "text", text }], isError: false };
}

function _error(text: any) {
