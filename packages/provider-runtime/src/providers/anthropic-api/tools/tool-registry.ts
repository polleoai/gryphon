 ...
 */
function getActiveTools(opts: Record<string, any> = {}) {
 ...
 */
function getToolSchemas(opts: Record<string, any> = {}) {
 ...
 */
async function executeTool(name: any, input: any, ctx: any) {
  ...
  try {
    ...
    return {
      content: [{
        type: "text",
        text: `Error executing ${name}: ${(e as Error).message || String(e)}`,
      }],
      isError: true,
    };
  }
