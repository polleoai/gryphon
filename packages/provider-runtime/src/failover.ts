...
type FailureKind = "availability" | "content" | "runtime";
type FailureReason =
  | "no-api-key" | "auth" | "insufficient-credit"
  | "quota" | "rate-limit" | "construct-null" | null;
...
const NO_API_KEY_RE =
  /could not resolve authentication|environment variable is missing|api[\s_-]?key[^.]{0,40}\b(?:not set|is missing|missing|is required|must be set|not provided|not configured|is empty)\b|\b(?:no|missing)\s+api[\s_-]?key\b/i;

// auth: a key exists but is invalid / expired / forbidden.
const AUTH_RE =
  /invalid[\s_-]?(?:x-)?api[\s_-]?key|api key not valid|invalid_api_key|authentication[\s_-]?(?:error|failed)|unauthorized|permission[\s_-]?denied|forbidden|expired/i;
...
const INSUFFICIENT_RE =
  /insufficient[\s_-]?(?:quota|credit|funds|balance)|insufficient_quota|billing|credit balance is too low|exceeded your current quota|check your plan|payment required|out of (?:credit|funds)|balance is too low/i;

// rate-limit: a transient throttle.
const RATE_LIMIT_RE =
  /\b429\b|too many requests|rate[\s\-_]?limit(?:ed)?|resource_exhausted|usage limit reached|[Pp]lease retry in/i;
...
const CONTENT_RE =
  /no parseable json|did not match schema|schema validation|content[\s_-]?filter|response was blocked|safety/i;

function _statusOf(err: any): number | null {
  if (!err) return null;
  return (
    err.status ||
    err.statusCode ||
...
function _avail(reason: FailureReason) {
  return { kind: "availability" as FailureKind, reason };
}
 ...
 */
function classifyProviderFailure(errOrNull: any): { kind: FailureKind; reason: FailureReason } {
  ...
  if (errOrNull == null) return _avail("construct-null");
  ...
  if (NO_API_KEY_RE.test(msg)) return _avail("no-api-key");
  if (status === 401 || status === 403 || AUTH_RE.test(msg)) return _avail("auth");
  if (status === 402 || INSUFFICIENT_RE.test(msg)) return _avail("insufficient-credit");
  if (status === 429 || RATE_LIMIT_RE.test(msg)) return _avail("rate-limit");
  if (QUOTA_RE.test(msg)) return _avail("quota");
