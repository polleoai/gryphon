...
function parseAndValidate(text, schema) {
    ...
    try {
        ...
        throw new CliStructuredOutputError(`Model output is not valid JSON: ${e.message}`, { reason: "parse", lastOutput: cleaned });
    }
    const errors = validate(parsed, schema, "");
    if (errors.length > 0) {
        throw new CliStructuredOutputError(`Model output failed schema validation: ${errors.join("; ")}`, { reason: "validate", lastOutput: cleaned });
    }
    return parsed;
}
