...
const SCHEMA = {
    name: "Edit",
    description: "Performs exact string replacement in an existing file. The old_string " +
...
async function execute(input, ctx) {
    ...
    if (stat.size > maxSize) {
        ...
        return _error(`File too large to edit: ${filePath} is ${mb} MB (limit ${limitMb} MB). ` +
    ...
    if (BINARY_EXTS.has(ext)) {
        return _error(`Cannot edit binary file (${ext}): ${filePath}. Edit only supports ` +
    ...
    if (_looksBinary(rawBuf)) {
        return _error(`File appears to be binary (null bytes in the first 8 KB): ${filePath}. ` +
    ...
    if (occurrences === 0) {
        return _error(`old_string not found in ${filePath}. Read the file to see its actual content — ` +
            `the string may have different whitespace or line endings than you expect.`);
    }
    if (occurrences > 1 && !replaceAll) {
        return _error(`old_string matches ${occurrences} places in ${filePath}. Either provide more ` +
            `context to make it unique, or set replace_all=true to replace every occurrence.`);
    }
    const updated = replaceAll
        ? original.split(oldString).join(newString)
        : original.replace(oldString, newString);
    const classification = attackDetector.classify("Edit", { file_path: filePath, old_string: oldString, new_string: newString }, ctx);
    ...
    if (current !== original) {
        return _error(`File changed during permission approval: ${filePath}. ` +
    ...
    try {
        ...
        if (e.code === "ELOOP" || e.code === "EMLINK") {
            return _error(`Refusing to follow a symlink at the edit target: ${filePath}. ` +
    ...
    return _ok(`Edited ${filePath}: replaced ${replaceAll ? `${occurrences} occurrences` : "1 occurrence"} ` +
