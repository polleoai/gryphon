...
async function execute(input, ctx) {
    ...
    try {
        ...
        return _error(`Path escaped the vault between resolution and mkdir: ${filePath}. ` +
    ...
    const openFlags = fs.constants.O_WRONLY | fs.constants.O_CREAT | fs.constants.O_TRUNC | O_NOFOLLOW;
    let fd;
    try {
        ...
        if (e.code === "ELOOP" || e.code === "EMLINK") {
            return _error(`Refusing to follow a symlink at the write target: ${filePath}. ` +
    ...
    return _ok(`${exists ? "Updated" : "Created"} ${filePath} (${Buffer.byteLength(content, "utf8")} bytes, ${content.split("\n").length} lines)`);
}
function _ok(text) {
