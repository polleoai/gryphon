...
async function execute(input, ctx) {
    ...
    if (ctx.permissionMode === "plan") {
        return _error(`Plan mode is active — WebSearch is not permitted. Describe the ` +
    ...
    if (!apiKey) {
        return _error(`Brave Search API key not configured. SDK-mode WebSearch requires ` +
    ...
    if (!body && typeof response.text === "string") {
        try {
            ...
            return _error(`Brave Search returned non-JSON response: ${e.message}`);
        }
    }
    ...
    return _ok(`Top ${formatted.length} results for "${query}":\n\n` +
        formatted.join("\n\n"));
}
function _ok(text) {
