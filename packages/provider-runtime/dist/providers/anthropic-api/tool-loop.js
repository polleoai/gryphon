...
const GRYPHON_SDK_SYSTEM_PROMPT_BASE = "You are running inside the Gryphon Obsidian plugin. The user's " +
    ...
    while (iterations < MAX_ITERATIONS) {
        ...
        stream.on("text", (delta) => {
            ...
            if (callbacks.onMessage)
                callbacks.onMessage(turnText, "replace");
        });
        stream.on("error", (err) => {
        ...
        for (const block of toolUseBlocks) {
            ...
            if (coercionToolName && block.name === coercionToolName)
                continue;
            if (callbacks.onTool)
                callbacks.onTool(block.name);
