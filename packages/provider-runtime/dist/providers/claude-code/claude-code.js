...
const { GRYPHON_SYSTEM_PROMPT_HINT, GRYPHON_FALLBACK_DENY_HINT, } = require("@gryphon/protect");
...
class ClaudeCodeProvider {
    ...
    spawn() {
        ...
        const protectedModeOn = !!plugin && plugin.settings && plugin.settings.protectedMode !== false;
        const autoDenyProtected = !!plugin && plugin.settings && plugin.settings.autoDenyProtected === true;
        ...
        const hookPreflight = {
            ...
            hasAbsolutePluginDir: !!(plugin && typeof plugin.absolutePluginDir === "function" && plugin.absolutePluginDir()),
            hasNodeBinary: !!hookNodePath,
        };
        const enableHooks = hookPreflight.protectedModeOn &&
        ...
        if (enableHooks) {
            try {
                ...
                if (!fs.existsSync(ipcHelper))
                    missing.push(ipcHelper);
                if (missing.length > 0) {
                    ...
                    try {
                        ...
                        if (!fs.existsSync(hookDir) && fs.existsSync(pluginDir)) {
                            ...
                            const renamed = siblings.filter((name) => name !== "hooks" && conflictRe.some((r) => r.test(name)));
                            if (renamed.length > 0) {
                                cloudHint =
                    ...
                    throw new Error(`Gryphon plugin helper files not found — the plugin directory ` +
                ...
                // where to look.
                this.hostAdapter.notify(`Gryphon: advanced protections unavailable — plugin files are incomplete.\n\n${msg}`, { level: "error", timeoutMs: 20000 });
                if (hookSettingsFile && fs.existsSync(hookSettingsFile)) {
                    try {
                        fs.unlinkSync(hookSettingsFile);
                    }
                    catch (_) { /* ignore */ }
                }
            ...
            if (denyGlobs.length > 0) {
                try {
                    ...
                    if (autoDenyProtected) {
                        this.hostAdapter.notify(`Gryphon: couldn't apply protected-pattern enforcement — ` +
                            ...
                            `temp directory, then restart Gryphon.`, { level: "error", timeoutMs: 20000 });
                    }
                }
            }
        }
        ...
        if (this.options.extraArgs && Array.isArray(this.options.extraArgs)) {
            ...
            if (dropped.length > 0) {
                ...
                console.error(`[gryphon/claude-code] Dropped ${dropped.length} cross-provider flag(s) ` +
        ...
        proc.stdout.on("data", (data) => { if (forThisProcess())
            this._handleStdout(data); });
        proc.stderr.on("data", (data) => { if (forThisProcess())
            this._handleStderr(data); });
        proc.on("close", (code) => { if (forThisProcess())
            this._handleClose(code); });
        proc.on("error", (err) => { if (forThisProcess())
        ...
        if (options && options.structuredOutput) {
            ...
            const loop = async () => {
                ...
                try {
                    ...
                    throw new CliStructuredOutputError(`structured-output budget exhausted after ${maxRetries} attempts on claude-code: ${lastError && lastError.message}`, {
    ...
    _doOneTurn(prompt) {
        ...
        if (!this.process) {
            return Promise.reject(this._lastSpawnError || new Error("Claude CLI failed to start."));
        }
    ...
    _processEvent(raw) {
        ...
        if (raw.type === "stream_event") {
            ...
            if (event.type === "content_block_delta" &&
                ...
                if (this.onMessage)
                    this.onMessage(this.turnText, "replace");
            }
        ...
        if (raw.type === "assistant" && raw.message?.content) {
            for (const block of raw.message.content) {
                ...
                if (block.type === "tool_use") {
                    if (this.onMessage)
                        this.onMessage(block.name, "tool");
                }
        ...
        if (raw.type === "tool_use") {
            if (this.onMessage)
                this.onMessage(raw.name || "unknown", "tool");
            return;
        }
    ...
    _handleProcessError(err) {
        ...
        try {
            ...
            console.error("[gryphon/cli] process error:", {
                code: err && err.code,
                errno: err && err.errno,
