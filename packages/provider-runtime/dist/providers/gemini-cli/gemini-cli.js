...
const _UNSUPPORTED_CLIENT_MESSAGE = "Gemini CLI reports this account is no longer supported for Gemini Code " +
...
exports._UNSUPPORTED_CLIENT_MESSAGE = _UNSUPPORTED_CLIENT_MESSAGE;
...
function _formatRateLimitMessage(body) {
    ...
    if (seconds !== null && seconds < 1) {
        return (`Gemini returned a transient blip (retry suggested in ${Math.round(seconds * 1000)}ms). ` +
    ...
    const interestingLines = body
        .split(/\r?\n/)
        .map((l) => l.trim())
        .filter((l) => /quotaMetric|quotaId|quotaValue|quota_metric|RESOURCE_EXHAUSTED|generativelanguage|model=|quotaDimensions|GenerateRequestsPerDay|GenerateRequestsPerMinute|free_tier_requests/i.test(l))
        .slice(-6)
        .join("\n");
...
class GeminiCliProvider {
    ...
    _buildArgs(prompt, { hooksWired = false } = {}) {
        ...
        if (this.options.extraArgs && Array.isArray(this.options.extraArgs)) {
            ...
            if (dropped.length > 0) {
                ...
                console.error(`[gryphon/gemini-cli] Dropped ${dropped.length} cross-provider flag(s) ` +
        ...
        if (options && options.structuredOutput) {
            ...
            const loop = async () => {
                ...
                try {
                    ...
                    throw new CliStructuredOutputError(`structured-output budget exhausted after ${maxRetries} attempts on gemini-cli: ${lastError && lastError.message}`, {
    ...
    _handleStderr(data) {
        ...
        if (/UNSUPPORTED_CLIENT/.test(trimmed)) {
            ...
            if (this.onError)
                this.onError(_UNSUPPORTED_CLIENT_MESSAGE);
            return;
        }
        // Auth-missing case → surface a clean error.
        if (/Please set an Auth method|GEMINI_API_KEY/i.test(trimmed)) {
            if (this.onError)
                this.onError("Gemini CLI authentication is not set. Paste a Google API key in " +
    ...
    _handleStaleSession() {
        ...
        if (typeof prompt !== "string" || prompt.length === 0) {
            ...
            this._currentReject = null;
            if (reject)
                reject(new Error("Gemini resumed a session that no longer exists. Try the request " +
