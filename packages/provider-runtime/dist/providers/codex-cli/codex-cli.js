...
function _mapPermissionToSandbox(permissionMode) {
    ...
    if (target !== "danger-full-access" && !_supportsLandlockSandbox()) {
        if (!_landlockWarningEmitted) {
            ...
            console.warn("[gryphon/codex-cli] Linux kernel " + os.release() + " is pre-landlock " +
...
function _scrubInternalLeaks(text) {
    ...
    out = out.replace(/^(?:[\s>]*)?Command blocked by PreToolUse hook:\s*/gim, "");
    ...
    const _stripTrailerIfSafe = (s) => {
        ...
        if (tail.includes("```"))
            return s; // candidate is inside / before a code fence
...
class CodexProvider {
    ...
    _buildArgs(prompt) {
        ...
        if (this.options.extraArgs && Array.isArray(this.options.extraArgs)) {
            ...
            if (dropped.length > 0) {
                ...
                console.error(`[gryphon/codex-cli] Dropped ${dropped.length} cross-provider flag(s) ` +
        ...
        if (options && options.structuredOutput) {
            ...
            const loop = async () => {
                ...
                try {
                    ...
                    throw new CliStructuredOutputError(`structured-output budget exhausted after ${maxRetries} attempts on codex-cli: ${lastError && lastError.message}`, {
    ...
    _processEvent(raw) {
        ...
        if (raw.type === "item.started") {
            ...
            if (item.type === "command_execution") {
                if (this.onMessage)
                    this.onMessage("Bash", "tool");
            }
            else if (item.type === "file_change") {
                if (this.onMessage)
                    this.onMessage("Edit", "tool");
            }
            else if (item.type === "web_search") {
                if (this.onMessage)
                    this.onMessage("WebSearch", "tool");
            }
            else if (typeof item.type === "string" && item.type) {
                if (this.onMessage)
                    this.onMessage(item.type, "tool");
            }
            return;
        }
    ...
    _handleStaleSession() {
        ...
        if (typeof prompt === "string" && prompt.length > 0) {
            ...
            this._currentReject = null;
            if (reject)
                reject(new Error("Codex resumed a session that no longer has a rollout on disk. " +
    ...
    _handleStderr(data) {
        ...
        if (/Reading additional input from stdin/i.test(trimmed))
            return;
        if (/^[\d:T.Z-]+\s+ERROR codex_core::session: failed to load skill/i.test(trimmed))
            return;
        ...
        if (/not logged in|run [`'"]?codex login|authentication failed/i.test(trimmed)) {
            if (this.onError)
                this.onError("Codex CLI is not logged in. Run `codex login` in a terminal, then retry.");
            return;
        }
    }
    _handleClose(code) {
