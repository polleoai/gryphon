...
const DEFAULT_PROTECTED_PATHS = [
    ...
    },
    {
        ...
        userRisk: "This is git's configuration file. A write here can tell git to run a malicious command the next time you commit, push, or pull — silent code execution on your next git operation.",
    ...
    },
    {
        pattern: ".git/hooks/",
        category: "persistent-execution",
        userRisk: "Git hooks are shell scripts that run automatically on commit, push, or merge. A file placed here will execute the next time you do anything with git — silent, automatic code execution.",
    ...
    },
    {
        pattern: ".claude/",
        category: "persistent-execution",
        userRisk: "This folder holds Claude Code's project-level settings and hooks. A file placed here can automatically run on your next Claude Code session, giving an attacker a foothold that survives this conversation.",
    ...
    },
    {
        ...
        category: "persistent-execution",
        userRisk: "This is Claude Code's hook-definitions file. Entries here run shell commands whenever Claude Code sees a tool event — which fires the next time you use Claude Code anywhere.",
    ...
    },
    {
        ...
        category: "persistent-execution",
        userRisk: "VS Code auto-runs tasks marked for folder-open. A task placed here will run the next time you open this folder in VS Code — silently, without prompting.",
    ...
    },
    {
        ...
        category: "persistent-execution",
        userRisk: "This file controls how VS Code starts debug sessions. A modified pre-launch or post-debug task runs every time you debug — persistent code execution.",
...
const DEFAULT_PROTECTED_COMMANDS = [
    {
        ...
        userRisk: "A pipeline that feeds one command's output straight into a shell (or python / ruby / perl / node). This is the usual way to run code that came from somewhere else — if the \"somewhere else\" is a webpage or a data file that was fetched from the internet, it's running attacker-chosen code on your computer.",
    ...
    },
    {
        ...
        category: "runs-arbitrary-code",
        userRisk: "Same as pipe-to-shell, but the shell binary is reached indirectly — via a path (`/bin/sh`), a quoted name (`\"sh\"`), or a backslash-newline line continuation. Functionally identical risk; just a different surface form.",
    ...
    },
    {
        ...
        pattern: "\\|\\s*\\$\\{?SHELL\\}?\\b",
        category: "runs-arbitrary-code",
        userRisk: "A pipeline feeding command output into `$SHELL` — whatever your login shell is. Same effect as piping into bash or sh directly, just disguised.",
    ...
    },
    {
        ...
        category: "runs-arbitrary-code",
        userRisk: "This downloads a file from the web and runs it right away with your permissions. It's how many legitimate installers work — and also how many malware attacks start.",
    ...
    },
    {
        ...
        userRisk: "A recursive delete — removes a folder and everything inside it, permanently. Files removed this way are gone for good, not sent to the trash.",
    ...
    },
    {
        ...
        category: "persistent-execution",
        userRisk: "A shell redirect (`>`, `>>`, or `tee`) that writes directly into a protected settings folder — Obsidian, git, Claude Code, or VS Code. This is the \"back door\" way to change those settings or plant hidden code without Gryphon's normal file-write check seeing it.",
    ...
    },
    {
        ...
        platforms: ["windows"],
        category: "accesses-system",
        userRisk: "A shell redirect that writes into a Windows system, user-profile, or Claude Code configuration directory (like `AppData`, `Windows`, `ProgramData`, `System32`, `Program Files`, or `.claude`). These paths sit outside your Obsidian vault and can affect your whole computer, or your other Claude sessions.",
    ...
    },
    {
        ...
        platforms: ["windows"],
        category: "persistent-execution",
        userRisk: "`schtasks` manages Windows scheduled tasks. Creating or modifying one schedules code to run later — on a timer, on logon, or on a system event — outside this conversation where you can see it.",
    ...
    },
    {
        ...
        platforms: ["windows"],
        category: "persistent-execution",
        userRisk: "PowerShell cmdlets that install, modify, or remove Windows scheduled tasks. The scheduled code runs later — on a timer, on logon, or on a system event — outside this conversation.",
    ...
    },
    {
        ...
        userRisk: "Launches a new process with administrator rights via a UAC prompt. On approval, the new process runs with full admin privileges — the Windows counterpart of `sudo`.",
    ...
    },
    {
        ...
        platforms: ["windows"],
        category: "persistent-execution",
        userRisk: "`sc.exe create` installs a new Windows service; `config` changes an existing one; `delete` removes one. Services survive reboots and can run at startup with elevated privileges — a classic persistence mechanism.",
    ...
    },
    {
        ...
        platforms: ["windows"],
        category: "persistent-execution",
        userRisk: "`New-Service` installs a new Windows service via PowerShell. Services survive reboots and can run at startup — same persistence concern as `sc.exe create`.",
    ...
    },
    {
        ...
        platforms: ["windows"],
        category: "accesses-system",
        userRisk: "PowerShell is writing to a Windows system, user-profile, or Claude Code configuration directory (`AppData`, `Windows`, `ProgramData`, `System32`, `Program Files`, `.claude`) via a native cmdlet. Same blast radius as a `>` redirect — can affect your whole computer or your other Claude sessions.",
    ...
    },
    {
        ...
        platforms: ["windows"],
        category: "runs-arbitrary-code",
        userRisk: "`schtasks /run` triggers an already-installed Windows scheduled task immediately. If the task points at malicious code, running it via this command bypasses the \"scheduled later\" signal — it's effectively arbitrary-code-execution through the task scheduler.",
    ...
    },
    {
        ...
        category: "runs-arbitrary-code",
        userRisk: "A pipeline feeding command output into `env <interpreter>`. Same as piping directly into the interpreter — attacker-chosen code runs with your permissions. The `env` wrapper is a common way to find the interpreter on PATH without an absolute path.",
    ...
    },
    {
        ...
        platforms: ["posix"],
        category: "persistent-execution",
        userRisk: "Marks a file as executable. Usually a step in a pattern where a script is written and then run. By itself harmless, but rarely needed in a normal chat.",
    ...
    },
    {
        ...
        category: "persistent-execution",
        userRisk: "Modifies git's configuration or hooks. Once changed, the malicious settings stay and fire automatically on your next git operation (commit, push, pull) — silent, persistent code execution.",
    ...
    },
    {
        ...
        platforms: ["posix"],
        category: "escalates-privileges",
        userRisk: "A command asking for administrator access to your computer. Very few normal chat requests need admin — mainly installers or system tweaks.",
    ...
    },
    {
        ...
        platforms: ["posix"],
        category: "runs-arbitrary-code",
        userRisk: "`eval` takes a string and runs it as a fresh shell command. Whatever string follows — possibly constructed from attacker-controlled data — becomes the command. Very rarely legitimate.",
    ...
    },
    {
        ...
        category: "runs-arbitrary-code",
        userRisk: "Running a shell with `-c` executes whatever inline script follows. Same risk as piping into a shell — just a different syntax for running the same kind of untrusted code.",
    ...
    },
    {
        ...
        category: "runs-arbitrary-code",
        userRisk: "Running python, ruby, perl, or node with `-c`, `-e`, or `--eval` tells it to run whatever code follows right there in the command. Same risk as `bash -c` — any code chosen by whoever set it up runs with your permissions.",
    ...
    },
    {
        ...
        category: "runs-arbitrary-code",
        userRisk: "An interpreter reading its script from stdin redirection (`python < file`). The contents of `file` run as code with your permissions — same risk as `python -c` with the contents inline.",
    ...
    },
    {
        ...
        category: "runs-arbitrary-code",
        userRisk: "A shell running the output of another command as a script via process substitution (`bash <(curl ...)`). Same risk as `curl | bash` — attacker-chosen code runs with your permissions.",
    ...
    },
    {
        ...
        platforms: ["posix"],
        category: "runs-arbitrary-code",
        userRisk: "`xargs` runs a command for every line of its input. If that input came from a file or a web page, xargs effectively becomes a loop that runs attacker-chosen commands.",
    ...
    },
    {
        ...
        platforms: ["posix"],
        category: "runs-arbitrary-code",
        userRisk: "`find -exec` runs the given command for every matched file. The same command can execute dozens or hundreds of times, possibly over sensitive files.",
    ...
    },
    {
        ...
        userRisk: "An ssh command with a remote command attached — runs that command on another computer using your ssh credentials. An interactive `ssh host` on its own is fine; this form is different, because the remote command is fixed in advance.",
    ...
    },
    {
        ...
        platforms: ["posix"],
        category: "persistent-execution",
        userRisk: "`nohup` and `setsid` let a command keep running after you close your terminal or log out. A common way to hide a long-running process that an attacker would want to keep alive after you stop paying attention.",
    ...
    },
    {
        ...
        platforms: ["posix"],
        category: "persistent-execution",
        userRisk: "Schedules code to run later — outside this conversation where you can see it. Attackers like this because the code fires when you're not watching, sometimes hours or days later.",
    ...
    // ──────────────────────────────────────────────────────────────────
    {
        ...
        userRisk: "`curl` with a URL fetches content from the internet. In an Obsidian knowledge-management session, downloading from the web is rarely the user's actual goal — it's more often the first step of a download-and-execute shape. Approve if you specifically asked for it; otherwise this is suspicious.",
    ...
    },
    {
        pattern: "\\bwget\\b[^|\\r\\n]{0,512}://",
        category: "network-fetch",
        userRisk: "`wget` with a URL fetches content from the internet. Same concern as `curl http://...` — download-and-execute patterns start with a fetch.",
    ...
    },
    {
        ...
        platforms: ["windows"],
        category: "network-fetch",
        userRisk: "PowerShell web cmdlet with a URL — `Invoke-WebRequest` (`iwr`) or `Invoke-RestMethod` (`irm`) fetches content from the internet. Common first step in Windows malware delivery.",
    ...
    },
    {
        ...
        platforms: ["windows"],
        category: "network-fetch",
        userRisk: "`Start-BitsTransfer` uses the Windows Background Intelligent Transfer Service to download a file. A less-observed download primitive favored for low-profile persistence. Rarely legitimate in a chat-driven workflow.",
    ...
    },
    {
        ...
        platforms: ["windows"],
        category: "network-fetch",
        userRisk: "Creating a `.NET` `WebClient` object is the first half of the classic `(New-Object Net.WebClient).DownloadString(...)` malware pattern. Pairs with a `.DownloadString()` / `.DownloadFile()` / `IEX` call to become download-and-execute.",
    ...
    },
    {
        ...
        platforms: ["windows"],
        category: "network-fetch",
        userRisk: "`certutil -urlcache` downloads a file from a URL and caches it locally. Intended for cert management, but widely abused as a living-off-the-land download primitive because it bypasses naïve \"no curl\" restrictions.",
    ...
    },
    {
        ...
        platforms: ["posix"],
        category: "network-fetch",
        userRisk: "A BSD-era or alternative downloader (`fetch`, `axel`, `aria2c`) with a URL. Same concern as `curl` / `wget` — content from the internet is arriving on your machine.",
    ...
    },
    {
        ...
        category: "runs-arbitrary-code",
        userRisk: "Reads a script and runs it in your current shell session — including any functions, aliases, or path changes it defines. Covers POSIX `source`/`. script.sh` AND PowerShell dot-source (`. script.ps1`) and invoke-operator (`& script.ps1`). If the script is attacker-controlled, those changes persist for the rest of the session.",
    ...
    },
    {
        ...
        platforms: ["windows"],
        category: "runs-arbitrary-code",
        userRisk: "`Invoke-Expression` / `IEX` runs arbitrary PowerShell code passed as a string. It's the single most-abused primitive in Windows malware — used in `IEX (New-Object Net.WebClient).DownloadString(...)` and similar download-and-execute shapes. Rarely legitimate in a knowledge-management session.",
    ...
    },
    {
        ...
        platforms: ["windows"],
        category: "runs-arbitrary-code",
        userRisk: "Launches PowerShell to run inline code (`-Command`) or a base64-encoded payload (`-EncodedCommand`). The encoded form is particularly concerning — it's unreadable in the command view, so whatever it runs is invisible to you at the approval moment.",
    ...
    // ──────────────────────────────────────────────────────────────────
    {
        ...
        platforms: ["posix"],
        category: "accesses-system",
        userRisk: "A file copy / move / link / install targeting a sensitive system directory — `/etc/`, `/usr/`, `/System/`, `/var/`, `/root/`, or a hidden settings folder like `~/.ssh/`, `~/.bashrc`, `~/.claude/`. These paths affect your whole computer, your other Claude sessions, or run automatically the next time you open a terminal.",
    ...
    },
    {
        ...
        platforms: ["posix"],
        category: "accesses-system",
        userRisk: "In-place editing of a file in a sensitive system directory via `sed -i`. Same blast radius as writing over `/etc/passwd` — changes persist and can affect system behavior or other Claude sessions.",
    ...
    },
    {
        ...
        platforms: ["windows"],
        category: "accesses-system",
        userRisk: "A PowerShell file-mutation cmdlet (`Copy-Item`, `Move-Item`, `New-Item`, `Rename-Item`) targeting a sensitive Windows directory. Same concerns as the redirect-into-AppData/Windows rule, just for the shape where the path is an argument.",
    ...
    },
    {
        ...
        platforms: ["windows"],
        category: "persistent-execution",
        userRisk: "A PowerShell cmdlet writing to the Windows registry. Like `reg add`, registry changes can survive reboots and affect which programs run at startup or how the system behaves.",
    ...
    },
    {
        ...
        platforms: ["posix"],
        category: "destructive-operation",
        userRisk: "`shred` overwrites a file's contents multiple times before removing it — unrecoverable by forensic tools. `unlink` removes the file-to-name link immediately, same blast radius as `rm` for the named file. Both are destructive; files do not go to the Trash.",
    ...
    },
    {
        ...
        platforms: ["posix"],
        category: "destructive-operation",
        userRisk: "`truncate -s 0 <file>` wipes the file's contents to zero bytes. Functionally a destructive write — previous contents are lost.",
    ...
    },
    {
        ...
        platforms: ["posix"],
        category: "destructive-operation",
        userRisk: "`dd` writing to a raw disk or partition device (e.g. `/dev/sda`, `/dev/nvme0n1`, `/dev/disk2`) can wipe or corrupt the storage device. This is one of the few commands that can render the system unbootable if pointed at the wrong device.",
    ...
    },
    {
        ...
        platforms: ["posix"],
        category: "destructive-operation",
        userRisk: "`find ... -delete` removes every file that matches the criteria in a single pass. The match set can be large and hard to review — one typo in the filter can delete much more than intended.",
    ...
    // ──────────────────────────────────────────────────────────────────
    {
        ...
        platforms: ["windows"],
        category: "runs-arbitrary-code",
        userRisk: "`mshta` executes HTML Application (.hta) files, which can contain JavaScript / VBScript. Widely abused by Windows malware as a script-execution primitive that bypasses many naïve restrictions. Extremely rare in legitimate workflows.",
    ...
    },
    {
        ...
        platforms: ["windows"],
        category: "runs-arbitrary-code",
        userRisk: "`rundll32` with a `javascript:` URL executes inline JavaScript in the Windows scripting host — a bypass primitive that sidesteps many AV hooks. The URL variant (`rundll32 <url>`) is a download-and-execute shape.",
    ...
    },
    {
        ...
        platforms: ["windows"],
        category: "runs-arbitrary-code",
        userRisk: "`regsvr32` with `scrobj.dll` or `/i:<url>` runs a remote scriptlet — a well-known bypass primitive (Squiblydoo) that many AV hooks miss. The scriptlet contents come from wherever the URL points to.",
    ...
    },
    {
        ...
        platforms: ["windows"],
        category: "runs-arbitrary-code",
        userRisk: "`wmic process call create` spawns an arbitrary process via WMI — effectively a shell without going through cmd.exe or PowerShell. A classic LOLBin for bypassing process-chain monitoring.",
...
const SLASH_COMMANDS = [
    ...
    { cmd: "/doctor", desc: "Run diagnostics — versions, providers, hooks, network reachability" },
    ...
    { cmd: "/recap", desc: "Summarize the conversation as an inline note without resetting" },
    { cmd: "/settings", desc: "Open plugin settings" },
    { cmd: "/status", desc: "Show session status — model, provider, perm, tokens, cost, messages" },
    { cmd: "/stop", desc: "Stop the current turn" },
    { cmd: "/usage", desc: "Show session usage (cost, tokens, messages, duration)" },
    { cmd: "/version", desc: "Show plugin version, provider, model, OS, Obsidian version" },
];
...
const RESERVED_SKILL_NAMES = new Set(SLASH_COMMANDS.map((c) => c.cmd.replace(/^\//, "")));
 ...
 */
function visibleProviderPrefs(prefs, current) {
