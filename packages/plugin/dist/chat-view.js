...
function _dedupeConsecutiveParagraphs(text) {
    ...
    for (const p of paragraphs) {
        ...
        if (prev !== undefined && prev === p && p.length >= 40)
            continue;
        out.push(p);
    }
    return out.join("\n\n");
...
function _collapseSummaryDrafts(text) {
    ...
    if (paragraphs.length < 3)
        return text; // need draft + final + deny
    ...
    for (let i = 0; i < paragraphs.length; i++) {
        for (const marker of _CANONICAL_BLOCK_MARKERS) {
            if (paragraphs[i].includes(marker)) {
                denyIdx = i;
                break;
            }
        }
    ...
    if (denyIdx < 1)
        return text; // <0: no deny; ==0: deny is first, no drafts to collapse
    ...
    for (const group of groups) {
        ...
        out.push(`<details class="gryphon-draft-fold"><summary>${summaryLabel}</summary>\n\n` +
            drafts.join("\n\n") +
            `\n\n</details>`);
        out.push(finalP);
    }
...
function _parseRetryAfterSeconds(err) {
    ...
    const headers = (err.headers && (typeof err.headers.get === "function"
...
function _replaceNoResponsePlaceholder(text) {
    ...
    if (text.trim() !== _CC_NO_RESPONSE_REQUESTED)
        return text;
    return ("_(The model returned no text response — this often happens on tool-only " +
...
function computeContextPct(tokens, windowSize) {
    if (!tokens || !windowSize || windowSize <= 0)
        return 0;
 ...
 * @returns {boolean}
 */
function shouldStartAutoCompact({ pct, isSdk, autoCompactSdk, isCompacting, messageCount, lagFailsafe, }) {
    if (isCompacting)
        return false;
    if (!isSdk)
        return false;
    if (autoCompactSdk === false)
        return false;
    if (!lagFailsafe && (pct == null || pct < 95))
        return false;
...
function nextContextWarningState(prev, pct) {
    ...
    if (pct >= 80 && pct < 95 && !shown)
        return { shown: true, fire: true };
    if (pct < 75 && shown)
        return { shown: false, fire: false };
...
function modelButtonText(settingsOrPlugin) {
    ...
    if (kind === "openai-api" || kind === "codex-cli") {
        ...
        const { getModelDropdownOptions, getCodexCliModelDropdownOptions, resolveModel: resolveOpenAIModel, coerceToCodexCliModel, DEFAULT_MODEL: OPENAI_DEFAULT_MODEL, CODEX_CLI_DEFAULT_MODEL, } = require("@gryphon/provider-runtime").pricing.openai;
...
function modelButtonTitle(settingsOrPlugin) {
    ...
    if (kind === "google-api" || kind === "gemini-cli")
        return "Model (Gemini)";
    if (kind === "anthropic-api" || kind === "claude-code")
        return "Model (Claude)";
    return "Model";
}
...
class GryphonChatView extends ItemView {
    constructor(leaf, plugin, options = {}) {
        ...
        for (const m of ["saveSettings"]) {
            if (!plugin || typeof plugin[m] !== "function") {
                console.error(`[gryphon] embedding host plugin is missing required method ${m}() — ` +
        ...
        for (const key of Object.keys(this.extraProcessArgsByProvider)) {
            if (!knownSet.has(key)) {
                console.error(`[gryphon] extraProcessArgsByProvider has unknown provider key ` +
    ...
    async onOpen() {
        ...
        setTooltip(this.contextBtn, "How much of the model's context window your last prompt used.\n\n"
            ...
            + "\"Prompt is too long.\"", { placement: "bottom" });
        ...
        setTooltip(this.projectionBtn, "Gryphon's estimate of how much of the model's context window\n"
            ...
            + "typing, and whenever you switch models.", { placement: "bottom" });
        toolbar.createEl("span", { text: "·", cls: "gryphon-toolbar-sep" });
        ...
        this.inputEl.addEventListener("keydown", (e) => {
            ...
            if (this.autocompleteEl.style.display !== "none") {
                ...
                if ((e.key === "Enter" || e.key === "Tab") && this.autocompleteIdx >= 0) {
                    ...
                    if (selected)
                        this._selectAcItem(selected.dataset.cmd);
                    return;
                }
        ...
        if (this.app && this.app.workspace && typeof this.app.workspace.on === "function") {
            this.registerEvent(this.app.workspace.on("gryphon:settings-changed", () => {
    ...
    _teardownLiveProcessIfSettingsChanged() {
        ...
        if (!alive)
            return;
        if (!this._providerSignatureChanged())
            return;
        try {
    ...
    _walkPromptHistory(direction, currentText) {
        ...
        // direction === "forward"
        if (!inHistoryMode)
            return; // nothing newer to show when not in history
    ...
    async _applyDirectSetting(settingKey, newValue, list, btnEl, displayLabel) {
        ...
        if (!valid) {
            this._flashStatus(`Unknown ${displayLabel.toLowerCase()}: ${newValue} \u2014 valid: ${list.map((x) => x.value).join(", ")}`);
            return;
        }
        this.plugin.settings[settingKey] = newValue;
        await this.plugin.saveSettings();
        if (btnEl)
            btnEl.textContent = valid.label + " \u25BE";
        if (this.claudeProcess && this.claudeProcess.isAlive()) {
    ...
    _currentCalibrationDelta() {
        ...
        if (!this._calibrationMissingWarned) {
            ...
            console.warn("[gryphon] calibration sample API missing on plugin instance — " +
    ...
    _updateRestApiChip() {
        ...
        setTooltip(this.restApiBtn, blocked
            ? "REST API: off.\n\n"
    ...
    updateContextMeter(contextTokens) {
        ...
        if (contextTokens > 0)
            this._lastMeasuredTokens = contextTokens;
        ...
        if (contextTokens > 0
            ...
            if (Math.abs(delta) < windowSize / 2 && typeof this.plugin.recordProjectionCalibrationSample === "function") {
                ...
                // Issue #4 — guard the delta read by its OWN presence, not the
                // sibling's: a host that implements recordProjectionCalibrationSample
                // but not getProjectionCalibrationDelta must not throw here.
                if (typeof this.plugin.getProjectionCalibrationDelta === "function") {
        ...
        setTooltip(this.contextBtn, `Your last prompt used ${usedK}K — ${pct}% of the ${winK}K context window (${remK}K still free).\n\n`
        ...
        if (pct >= AUTO_COMPACT_SDK_THRESHOLD_PCT)
            this.contextBtn.addClass("gryphon-context-danger");
        else if (pct >= CONTEXT_WARN_PCT)
            this.contextBtn.addClass("gryphon-context-warn");
    ...
    _refreshMeasuredAgainstCurrentModel() {
        const liveTokens = this.claudeProcess && typeof this.claudeProcess.contextTokens === "number"
    ...
    _updateProjectionChip(result) {
        ...
        setTooltip(this.projectionBtn, `Gryphon estimates your next prompt at ${prefix}${totalK}K — ${prefix}${result.rawPct}% of the ${windowK}K context window. ${modeLine}\n\n`
            ...
            + "confirm before sending.", { placement: "bottom" });
    ...
    _maybeFlashProjectionWarning(result) {
        if (!result || result.pct < CONTEXT_WARN_PCT)
            return;
        if (this._projectionWarningShown)
            return;
        this._projectionWarningShown = true;
        const k = Math.round(result.totalTokens / 1000);
        const winK = Math.round(result.windowSize / 1000);
        this._flashStatus(`Next send projected at ${result.pct}% (${k}K / ${winK}K). `
    ...
    _isSdkMode() {
        ...
        if (sid)
            return String(sid).startsWith("sdk-");
    ...
    async handleChatCommand(text) {
        ...
        const handlers = [
            ...
            { match: (c) => c === "/cost" || c.startsWith("/cost "), run: () => this._flashStatus(`Session cost: $${this.cumulativeCost.toFixed(4)}${this._costSuffix()}`) },
            { match: (c) => c === "/usage", run: () => this._cmdShowUsage() },
            { match: (c) => c === "/model", run: () => this.modelBtn && this.showModelMenu({ target: this.modelBtn }) },
            { match: (c) => c.startsWith("/model "), run: () => this._applyDirectSetting("model", text.trim().substring(7).trim(), MODELS, this.modelBtn, "Model") },
            { match: (c) => c === "/effort", run: () => this.effortBtn && this.showEffortMenu({ target: this.effortBtn }) },
            { match: (c) => c.startsWith("/effort "), run: () => this._applyDirectSetting("effort", text.trim().substring(8).trim(), EFFORTS, this.effortBtn, "Effort") },
            { match: (c) => c === "/perm" || c === "/permissions", run: () => this.permBtn && this.showPermMenu({ target: this.permBtn }) },
            { match: (c) => c === "/stop", run: () => this.isStreaming ? this.stopStreaming() : this._flashStatus("Nothing to stop") },
            ...
            { match: (c) => c === "/btw", run: () => this._flashStatus("/btw needs a note: type /btw <your side note>") },
        ];
        ...
        if (CC_BLOCKED_IN_STREAM_JSON.has(first)) {
            this._flashStatus(`${first} isn't supported in Gryphon \u2014 use the local CLI's own terminal for that`);
            return true;
        }
    ...
    _confirmOverflowSend(projection) {
        ...
        return new Promise((resolve) => {
            ...
            modal.contentEl.createEl("p", {
                text: `This send is projected at ${pctLabel} of the model's window `
            ...
            new Setting(modal.contentEl)
                ...
                .addButton((btn) => btn.setButtonText("Send anyway").setWarning().onClick(() => finish(true)));
    ...
    async _showContextBreakdownModal() {
        ...
        if (!result || typeof result.totalTokens !== "number") {
            modal.contentEl.createEl("p", {
                text: "Gryphon hasn't built a context snapshot for this view yet. "
        ...
        modal.contentEl.createEl("p", {
            text: `Projected: ${totalK}K of ${winK}K context window (${pctLabel}).`,
    ...
    _confirmClear() {
        ...
        return new Promise((resolve) => {
            ...
            modal.contentEl.createEl("p", {
                text: "This deletes the entire chat history for this session and " +
            ...
            new Setting(modal.contentEl)
                .addButton((btn) => btn.setButtonText("Cancel").onClick(() => finish(false)))
                .addButton((btn) => btn.setButtonText("Clear").setWarning().onClick(() => finish(true)));
            modal.onClose = () => finish(false);
            modal.open();
        });
    }
    ...
    async _cmdCompact(opts = {}) {
        ...
        const summaryPrompt = "Generate a comprehensive summary of our conversation that will REPLACE the full message history. Future turns will use only this summary plus new messages as context \u2014 so be thorough.\n\n" +
            "Include:\n" +
    ...
    _buildDiagnosticContext() {
        ...
        const provider = (() => {
            const sid = this.claudeProcess && this.claudeProcess.sessionId;
            if (sid && String(sid).startsWith("sdk-"))
    ...
    _cmdVersion() {
        ...
        const text = `**Gryphon v${d.pluginVersion}**\n` +
    ...
    async _cmdRecap() {
        ...
        const recapPrompt = "Generate a concise recap of our conversation so far — what we've " +
    ...
    async _cmdInitManual() {
        ...
        if (fs.existsSync(manualAbs)) {
            ...
            try {
                this.app.workspace.openLinkText(manualRel, "");
            }
            catch { }
            return;
        }
        const template = "# Gryphon — Vault Manual\n\n" +
    ...
    _openFeedbackMailto(userText) {
        ...
        const body = encodeURIComponent(`${userText || "(write your feedback here)"}\n\n---\n${this._buildFeedbackDiagText()}\n`);
    ...
    _openFeedbackIssueTracker(userText) {
        ...
        const body = encodeURIComponent(`${userText || "Describe the issue here…"}\n\n` +
            `**Steps to reproduce:**\n1. \n2. \n\n` +
            `**Expected:**\n\n**Actual:**\n\n` +
            `---\n${this._buildFeedbackDiagText()}\n`);
    ...
    _showFeedbackModal() {
        ...
        note.setText("Gryphon never auto-sends — every option opens a draft you review " +
        ...
        new Setting(modal.contentEl)
            ...
            modal.close();
            try {
                window.open("https://github.com/polleoai/gryphon/issues");
            }
            catch { }
        }));
        modal.open();
    ...
    _cmdShowHelp() {
        ...
        body.createEl("p", {
            text: "Type /<skill-name> to invoke a custom skill. Skills are .md files " +
    ...
    async _cmdMemoryAudit() {
        ...
        if (!memoryDir) {
            modal.contentEl.createEl("p", {
                text: "No Claude Code memory directory exists for this vault yet. "
            ...
            if (vaultPath && homeDir) {
                ...
                const expected = require("path").join(homeDir, ".claude", "projects", escapeVaultPath(vaultPath), "memory");
        ...
        if (indexFile && indexFile.size > 24 * 1024) {
            const warn = headerEl.createEl("div", {
                text: `MEMORY.md is ${(indexFile.size / 1024).toFixed(1)} KB — past CC's `
        ...
        if (candidates.length === 0) {
            modal.contentEl.createEl("p", {
                text: `No files older than ${ageThreshold} days. The memory tree is `
        ...
        archiveBtn.addEventListener("click", () => {
            ...
            try {
                if (moved.length < attempted) {
                    ...
                    new Notice(`Archived ${moved.length} of ${attempted} files — ${attempted - moved.length} couldn't be moved (see console).`, 8000);
                }
                if (indexResult && indexResult.indexWriteError) {
                    new Notice(`Gryphon archived the files but couldn't update MEMORY.md — ${indexResult.indexWriteError}. Open MEMORY.md and remove or comment out the archived entries manually.`, 12000);
                }
                ...
                if (moved.length > 0 && moved.length === attempted && !(indexResult && indexResult.indexWriteError)) {
                    const n = new Notice(`Archived ${moved.length} memory file${moved.length === 1 ? "" : "s"}. Click to undo.`, 5000);
                    ...
                    if (n && n.noticeEl) {
                        ...
                        n.noticeEl.addEventListener("click", () => {
                            ...
                            try {
                                new Notice(`Restored ${restored.length} of ${moved.length} files.`
    ...
    stopStreaming() {
        ...
        for (const hook of this.stopStreamingHooks) {
            try {
                ...
                console.warn("[gryphon] stopStreaming hook error:", e);
            }
        }
     ...
     */
    _clearQueuedPrompts() {
        if (!this._queuedPrompts || this._queuedPrompts.length === 0)
            return;
        ...
        for (const q of this._queuedPrompts) {
            if (q.bubbleEl && q.bubbleEl.parentElement)
                q.bubbleEl.remove();
        }
        this._queuedPrompts = [];
    ...
    _drainQueuedPrompts() {
        ...
        if (next.bubbleEl && next.bubbleEl.parentElement)
            next.bubbleEl.remove();
        ...
        if (idx >= 0)
            this._pendingQueuedTexts.splice(idx, 1);
        this._invalidatePromptHistoryCache();
    ...
    _maybeStartAutoCompact(opts = {}) {
        if (this._compactionPending || this._compactionInProgress)
            return false;
        if (!this._isSdkMode())
            return false;
        ...
        if (this.plugin.settings.autoCompactSdk === false) {
            if (overThreshold || opts.lagFailsafe) {
                this._flashStatus(`Context at ${pct}% — auto-compact disabled. Run /compact manually before the next send.`);
            }
            return false;
        }
        if (!opts.lagFailsafe && !overThreshold)
            return false;
        if (!this.messages || this.messages.length < 4)
            return false;
        ...
        const reasonNote = opts.lagFailsafe
            ? "context overflowed"
            : `${pct}%`;
        this._flashStatus(`Auto-compacting (${reasonNote}) — fresh session will start after this.`);
    ...
    _emergencyTrim(provider, keepRecent = 8) {
        if (!provider || !Array.isArray(provider.history))
            return false;
        if (provider.history.length <= keepRecent)
            return false;
        ...
        console.warn(`[gryphon] emergency-trim: dropped ${original - provider.history.length} ` +
    ...
    _extractLlmTurnsFromFullHistory() {
        ...
        const MAX_SDK_SEED_TURNS = Math.max(SEED_CAP_BASE, Math.round(SEED_CAP_BASE * (window / SEED_CAP_BASE_WINDOW)));
        ...
        if (!this._fullHistory || this._fullHistory.length === 0)
            return [];
        ...
        const llmOnly = history.filter((m) => m.source === "llm" && (m.role === "user" || m.role === "assistant"));
    ...
    _cmdResetContext() {
        ...
        this.scrollToBottom();
        this._flashStatus("Context cleared for next message — prior conversation stays visible.");
    }
    /**
    ...
    _stripContextBlock(text) {
        ...
        do {
            prev = text;
            text = text.replace(/^\s*\[gryphon-(?:context|reminder)\][\s\S]*?\[\/gryphon-(?:context|reminder)\]\s*/, "");
        } while (text !== prev);
        return text;
    }
    /**
    ...
    _buildTriggerKeywords() {
        ...
        for (const p of activeCmds) {
            ...
            for (const tok of tokens) {
                ...
                if (REMINDER_TRIGGER_EXCLUSIONS.has(lower))
                    continue;
                keywords.add(lower);
            }
        }
    ...
    _consumePostDenyClarifier() {
        if (!this._priorTurnHadProtectedDeny)
            return "";
        this._priorTurnHadProtectedDeny = false;
    ...
    _getPromptHistory() {
        ...
        for (const m of (this.messages || [])) {
            if (m.ts)
                merged.set(m.ts, m); // session wins on collision
        }
        const ordered = [...merged.values()].sort((a, b) => (a.ts || "").localeCompare(b.ts || ""));
        const prompts = [];
    ...
    async _doSaveChatHistory() {
        ...
        const filtered = filterMessagesForSave(this.messages, this.plugin.settings.lastSessionId);
                ...
                if (data.trim()) {
                    try {
                        ...
                        for (const m of parsed) {
                            if (!m.ts)
                                m.ts = "2000-01-01T00:00:00Z"; // backward compat
                        ...
                        if (migrated) {
                            try {
                                ...
                                console.warn("[gryphon] chat-history.json migrated: stripped [gryphon-context] from existing user messages (issue #35)");
                            }
                            catch (e) {
                                console.warn(`[gryphon] chat-history.json migrate write-back failed: ${e.message}`);
                            }
                        }
                    }
                    catch (parseErr) {
    ...
    _loadOlderMessages() {
        if (!this._fullHistory || this._historyLoadedUpTo <= 0)
            return;
        const BATCH = 30;
        ...
        for (const msg of batch) {
            const el = this._renderMessage(msg);
            if (el)
                this.messagesEl.insertBefore(el, firstChild);
        }
    ...
    _surfaceSkillLoadErrors() {
        ...
        if (!reg || typeof reg.getErrors !== "function")
            return;
        const errors = reg.getErrors();
        if (errors.length === 0)
            return;
        const list = errors.map((e) => `  • ${e.path}`).join("\n");
        this.addSystemMessage(`${errors.length} skill file${errors.length === 1 ? "" : "s"} failed to load:\n${list}\n` +
    ...
    _renderWelcomePanelIfNeeded() {
        ...
        panel.createEl("p", {
            text: "Gryphon connects Obsidian to Claude — read and edit your vault, " +
    ...
    _renderCliCard(parent, detected) {
        ...
        if (detected.cliPath) {
            ...
            card.createEl("p", {
                text: `A \`claude\` binary was detected at ${displayPath(detected.cliPath)}. Switching ` +
            ...
            card.createEl("p", {
                text: "No local `claude` binary detected. Claude Code is an advanced " +
    ...
    _renderSdkCard(parent, detected) {
        ...
        if (detected.apiKey) {
            ...
            card.createEl("p", {
                text: "Paste an Anthropic API key in settings. Adds credit-based usage " +
    ...
    _flashProviderChangeNotice(prevPreference, newPreference) {
        ...
        const droppedSuffix = stats.dropped > 0
            ...
            : "";
        this._flashStatus(`Prior conversation (last ${seed.length} turns) will continue with ${label}${droppedSuffix}. Run /new to start fresh.`, 6000);
    }
    /**
        ...
        this.app.workspace.iterateAllLeaves((leaf) => {
            ...
            if (view instanceof MarkdownView && view.editor) {
                try {
                    ...
                    if (sel)
                        found = { text: sel, file: view.file || null };
                }
                catch { }
            }
        });
    ...
    _buildContextPrefix() {
        ...
        if (!activeFile)
            return "";
        if (activeFile.path.startsWith("Gryphon/Exports/"))
            return "";
    ...
    _previewAcItem(el) {
        ...
        if (desc)
            this._flashStatus(desc);
        else if (cmd)
            this._flashStatus(cmd);
    }
    _restorePreAcStatus() {
    ...
    async sendMessage() {
        ...
        if (text.startsWith("/")) {
            if (await this.handleChatCommand(text)) {
                this.inputEl.focus();
                return;
            }
        }
        ...
        try {
            ...
            const responseText = _collapseSummaryDrafts(_separateCanonicalBlocks(_dedupeConsecutiveParagraphs(_replaceNoResponsePlaceholder(rawResponseText))));
            ...
            if (result) {
                ...
                if (result.contextTokens)
                    this.updateContextMeter(result.contextTokens);
            }
        }
        catch (err) {
            if (this._connTimeout) {
            ...
            if (_isRateLimitError(err) && originalRawText && !this.inputEl.value) {
                ...
                const autoRetry = this.plugin.settings.autoRetryOnRateLimit === true &&
                    retryAfter !== null &&
                ...
                if (autoRetry) {
                    ...
                    this._autoRetryTimeout = window.setTimeout(() => {
                        ...
                        if (!this.inputEl)
                            return;
                        if (this.inputEl.value !== originalRawText)
                            return;
                    ...
                    this._flashStatus("Your prompt was preserved — press Send again when the rate limit clears.", 6000);
                }
            }
