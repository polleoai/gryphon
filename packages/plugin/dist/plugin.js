...
const { isObsidianRestApiUrl, buildRestApiDenyReason, RestApiTurnCounter, } = require("./rest-api-policy");
// Deep provider sub-paths — use file-relative paths so esbuild bundles from
// src directly (bypasses the exports map; preserves bundle size). These
// modules are only loaded lazily at settings-validation time, not on every
// message send.
...
class GryphonSettingTab extends PluginSettingTab {
    ...
    display() {
        ...
        this._descToTooltip(new Setting(containerEl).setName("Provider"), "SDK uses the Anthropic API directly (recommended — pay-per-token, " +
        ...
        const cliSetting = this._descToTooltip(new Setting(containerEl).setName("Claude Code path"), "Leave empty for auto-detect (checks common locations + your " +
        ...
        this._descToTooltip(new Setting(containerEl).setName("Anthropic API key"), "Required for Anthropic API mode. Paste your key here — stored in plugin " +
            ...
            const key = (this.plugin.settings.anthropicApiKey || "").trim() ||
        ...
        this._descToTooltip(new Setting(containerEl).setName("OpenAI API key"), "Required for OpenAI API mode (v1.2.0). Paste your key here — " +
        ...
        this._descToTooltip(new Setting(containerEl).setName("Google API key"), "Required for Google Gemini API mode (v1.2.0). Paste your key " +
            ...
            .onClick(async () => {
            btn.setDisabled(true).setButtonText("Testing...");
            const key = (this.plugin.settings.googleApiKey || "").trim() ||
        ...
        this._descToTooltip(new Setting(containerEl).setName("Codex CLI path"), "Optional. Required only when Provider is Codex CLI. Empty string " +
            ...
            const codexPath = (this.plugin.settings.codexPath || "").trim() ||
                findCodexBinary() ||
                "";
        ...
        this._descToTooltip(new Setting(containerEl).setName("Gemini CLI path"), "Optional. Required only when Provider is Gemini CLI. Empty string " +
            ...
            const geminiPath = (this.plugin.settings.geminiCliPath || "").trim() ||
                findGeminiBinary() ||
                "";
        ...
        this._descToTooltip(new Setting(containerEl).setName("Brave Search API key"), "Optional. Enables SDK-mode WebSearch. Free at brave.com/search/api/ " +
        ...
        this._descToTooltip(new Setting(containerEl).setName("Auto-compact at 95% (SDK mode)"), "Automatically summarize and reset the conversation when context " +
            ...
            .addToggle((toggle) => toggle.setValue(this.plugin.settings.autoCompactSdk !== false).onChange(async (value) => {
        ...
        this._descToTooltip(new Setting(containerEl).setName("Auto-retry on rate-limit"), "When a send fails with a rate-limit error and the response " +
            ...
            "never auto-retried.")
            .addToggle((toggle) => toggle.setValue(this.plugin.settings.autoRetryOnRateLimit === true).onChange(async (value) => {
        ...
        // SDK-disabled path.
        this._descToTooltip(new Setting(containerEl).setName("Use exact token counts (SDK mode)"), "When using the Anthropic API provider, Gryphon calls the free " +
            ...
            .addToggle((toggle) => toggle.setValue(this.plugin.settings.useExactTokenCounting !== false).onChange(async (value) => {
        ...
        this._descToTooltip(new Setting(containerEl).setName("Confirm before overflow sends"), "When the projected context for the next send is at or above " +
            ...
            .addToggle((toggle) => toggle.setValue(this.plugin.settings.confirmOnContextOverflow !== false).onChange(async (value) => {
        ...
        this._descToTooltip(new Setting(containerEl).setName("Block Obsidian REST API access"), "When on (default), Gryphon denies WebFetch calls to the Obsidian " +
            ...
            .addToggle((toggle) => toggle.setValue((this.plugin.settings.obsidianRestApiPolicy || "blocked") === "blocked").onChange(async (value) => {
        ...
        this._descToTooltip(new Setting(containerEl).setName("Connection timeout (seconds)"), "How long to wait for the model's first token before treating " +
        ...
        if (activePref === "google-api" || activePref === "gemini-cli") {
            ...
            if (!isKnown) {
                const resolved = resolveModelFn(this.plugin.settings.model);
        ...
        this._descToTooltip(new Setting(containerEl).setName("Open in main tab"), "Open chat in the main editor area instead of the sidebar " +
            "(takes effect next time the chat is opened).")
            .addToggle((toggle) => toggle.setValue(this.plugin.settings.openInMainTab).onChange(async (value) => {
            this.plugin.settings.openInMainTab = value;
            await this.plugin.saveSettings();
        }));
        this._descToTooltip(new Setting(containerEl).setName("Max file size (MB)"), "Upper bound on the size of a single file that Read or Edit " +
        ...
        this._renderSectionHeading(containerEl, {
            ...
            tooltip: "Master switch for the protected-pattern feature.\n\n" +
        ...
        if (protectedModeOn) {
            ...
            this._renderSecurityHeaderRow(autoDenySection, {
                ...
                tooltipDetail: "ON: protected matches are refused immediately with a " +
        ...
        this._renderProtectedChecklist(protectedContainer, {
            ...
            tooltipDetail: "Off = no path-based approval modal at any mode; Claude Code's " +
            ...
            customHint: "Add your own folders or files here to protect irreplaceable content — " +
            ...
            customEmptyText: "Nothing added yet. Writes here always prompt — even in Safe or YOLO " +
        ...
        this._renderProtectedChecklist(protectedContainer, {
            ...
            tooltipDetail: "Off = no command-based approval modal at any mode; Claude Code's " +
                "active permission mode is the only gate. Your per-pattern " +
                "selections are preserved while disabled, so re-enabling " +
                "restores exactly the prior configuration.",
            ...
            customHint: "Add your own regex patterns for commands you always want to be " +
        ...
        if (this.plugin.provenanceStore) {
            ...
            this._renderSecurityHeaderRow(provSection, {
                ...
                tooltipDetail: "When Claude reads a tagged file, Gryphon appends a notice " +
            ...
            setTooltip(cleanBtn, "Remove tags for files that no longer exist on disk. Safe to run anytime.", { placement: "bottom" });
            ...
            setTooltip(clearBtn, "Remove every tag. Files themselves are not touched, but Claude no longer gets a data-not-instructions notice when re-reading them.", { placement: "bottom" });
        ...
        this._renderSectionHeading(containerEl, {
            title: "Diagnostics",
            tooltip: "Opt-in debug logging for troubleshooting and bug reports. " +
        ...
        this._descToTooltip(new Setting(containerEl).setName("CLI debug logging"), "When on, each CLI subprocess spawn logs its full argument " +
            ...
            .addToggle((toggle) => toggle.setValue(!!this.plugin.settings.devCliDebug).onChange(async (value) => {
        ...
        this._readOnDiskVersion().then((disk) => {
            ...
            if (disk === cachedVersion) {
                ...
                versionEl.setText(`Gryphon ${disk} (running). Obsidian's Community Plugins list still shows ${cachedVersion} — restart Obsidian to refresh that label.`);
            }
        }).catch(() => {
    ...
    _renderProtectedChecklist(containerEl, opts) {
        const { title, shortDesc, defaults, disabledKey, customKey, addPlaceholder, validateNew, customHint = null, customEmptyText = "No custom entries. Add one below.", enabledKey, manualAnchor, tooltipDetail, } = opts;
        ...
        this._renderSecurityHeaderRow(section, {
            ...
            onToggle: (value) => { if (applyEnabledVisual)
        ...
        const rerender = () => {
            ...
            if (!Array.isArray(settings[customKey]))
                settings[customKey] = [];
            ...
            for (const entry of visibleDefaults) {
                ...
                if (tooltip)
                    setTooltip(row, tooltip, { placement: "bottom" });
    ...
    _renderSectionHeading(parentEl, opts) {
        ...
        if (tooltip)
            this._descToTooltip(setting, tooltip);
        else
            setting.setDesc("");
...
class GryphonPlugin extends Plugin {
    async onload() {
        ...
        for (const hostId of GRYPHON_HOST_PLUGIN_IDS) {
            if (this.app.plugins.enabledPlugins.has(hostId)) {
                new Notice(`The "${hostId}" plugin embeds Gryphon — disable it to run ` +
                    `Gryphon standalone.`, 10000);
                return;
            }
        }
        ...
        try {
            ...
            if (s.totalRemoved > 0) {
                console.log(`[gryphon] orphan sweep: removed ${s.totalRemoved} file(s) ` +
        ...
        try {
            ...
            try {
                ...
                new Notice(`Gryphon: IPC server failed to start (${(e && e.message) || e}). ` +
        ...
        this.skillRegistry.init().catch((e) => console.warn("[gryphon] SkillRegistry init failed:", e));
        this.registerView(VIEW_TYPE, (leaf) => new GryphonChatView(leaf, this, {
            viewType: VIEW_TYPE,
            displayText: "Gryphon",
            icon: "shield-check",
        }));
    ...
    _pickSelectionForInjection() {
        ...
        if (mdView && mdView.editor) {
            ...
            if (sel)
                return { text: sel, file: mdView.file || null };
        }
    ...
    _announceProviderChange(prevPreference, newPreference) {
        if (!prevPreference || !newPreference)
            return;
        if (prevPreference === newPreference)
            return;
        for (const leaf of this.app.workspace.getLeavesOfType(VIEW_TYPE)) {
            ...
            if (view && typeof view._flashProviderChangeNotice === "function") {
                try {
                    view._flashProviderChangeNotice(prevPreference, newPreference);
                }
                catch (e) {
                    console.warn("[gryphon] provider-change notice failed:", e.message);
                }
            }
        }
    }
    ...
    _emitRefusalToChatViews(reason) {
        ...
        for (const leaf of this.app.workspace.getLeavesOfType(VIEW_TYPE)) {
            ...
            if (view && typeof view.addRefusalMessage === "function") {
                try {
                    view.addRefusalMessage(reason);
                }
                catch (e) {
                    console.warn("[gryphon] addRefusalMessage failed:", e.message);
                }
            }
        }
    }
    async onunload() {
        for (const leaf of this.app.workspace.getLeavesOfType(VIEW_TYPE)) {
            if (leaf.view.claudeProcess)
                leaf.view.claudeProcess.abort();
        }
    ...
    async ensureIpcListening(timeoutMs = 2000) {
        ...
        try {
            await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error("ipc-recovery-timeout")), timeoutMs)),
            ]);
            ...
            if (this.ipcServer && this.ipcServer.isListening())
                return true;
            console.warn("[gryphon] IPC recovery failed:", (e && e.message) || e, "— CLI spawn will proceed on fallback path");
            return false;
        }
    }
    /**
    ...
    _toVaultRelKey(p, cwd) {
        ...
        if (!vaultRoot || typeof p !== "string" || !p)
            return null;
        ...
        let normalizedRoot;
        try {
            normalizedRoot = fs.realpathSync(path.resolve(vaultRoot));
        }
        catch (_) {
            normalizedRoot = path.resolve(vaultRoot);
        }
    ...
    async _handleProvenanceMark(req) {
        ...
        if (!sid || !flag)
            return { ok: false, error: "missing sessionId/flag" };
    ...
    async _handleProvenanceAdd(req) {
        if (!this.provenanceStore)
            return { tagged: false, error: "store unavailable" };
        const key = this._toVaultRelKey(req && req.path, req && req.cwd);
        if (!key)
            return { tagged: false };
        const source = req && typeof req.source === "string" ? req.source : null;
        if (!source)
            return { tagged: false, error: "source required" };
        try {
    ...
    _openProvenanceListModal() {
        ...
        const entries = this.provenanceStore.list().sort((a, b) => a.path.localeCompare(b.path));
        if (entries.length === 0) {
    ...
    _confirmClearProvenance() {
        ...
        modal.contentEl.createEl("p", {
            text: "This removes every tag, but does NOT delete the files themselves. " +
        ...
        new Setting(modal.contentEl)
            ...
            .addButton((btn) => btn.setButtonText("Clear all").setWarning().onClick(() => {
            try {
    ...
    async _handleClassifyRequest(req) {
        ...
        const canonicalTool = (classification && classification.tool) ||
            attackDetector.normalizeToolName(tool) ||
            tool;
        const isMutating = canonicalTool === "Bash" || canonicalTool === "PowerShell" ||
    ...
    _onRestApiThresholdCrossed(count) {
        try {
            const { Notice } = require("obsidian");
            new Notice(`Gryphon: the model has issued ${count} Obsidian REST API GETs ` +
                `this turn. This pattern usually means enumeration — consider ` +
                `interrupting and asking it to use vault search instead.`, 12000);
        }
        catch (e) {
    ...
    _resetRestApiCounter() {
        if (this._restApiCounter)
            this._restApiCounter.reset();
    }
    /**
    ...
    consumeTaintedSession(rawSessionId) {
        if (!rawSessionId || typeof rawSessionId !== "string")
            return false;
        if (!this._taintedSessions.has(rawSessionId))
            return false;
    ...
    consumeForceFreshSpawn(providerKind) {
        if (!providerKind || typeof providerKind !== "string")
            return false;
        if (!this._forceFreshSpawnByProvider.has(providerKind))
            return false;
    ...
    _dropStalePerReloadSessionIds() {
        ...
        if (sid.startsWith("codex-cli-") || sid.startsWith("gemini-cli-")) {
            console.log(`[gryphon] dropping stale CLI lastSessionId on plugin load: ${sid} ` +
    ...
    recordProjectionCalibrationSample(deltaTokens) {
        if (!Number.isFinite(deltaTokens))
            return;
        const MAX = 20;
    ...
    async saveSettings() {
        ...
        if (w && typeof w.trigger === "function") {
            try {
                ...
                console.error(`[gryphon] gryphon:settings-changed listener threw — settings DID persist. ` +
            ...
            console.error("[gryphon] this.app.workspace.trigger missing — toolbar badges will " +
