...
class SkillRegistry {
    ...
    async _doLoadFile(file) {
        ...
        if (typeof basePath === "string" && basePath.length > 0) {
            try {
                ...
                if (rel === "" || rel.startsWith("..") || path.isAbsolute(rel)) {
                    this._recordError(file.path, "Refusing to load skill: the file resolves outside the vault via a symlink. " +
                        "Move the skill content into the vault directly.");
                    return;
                }
            }
            catch (e) {
                this._recordError(file.path, `Cannot resolve skill path (broken symlink or permission error): ${e.message}`);
                return;
            }
        }
        let parsed;
        try {
            ...
            this._recordError(file.path, `Frontmatter parse failed: ${e.message}. ` +
                `Skill files must start with a YAML block (--- ... ---) ` +
                `containing at least 'name' and 'description'.`);
            return;
        }
        if (this._unloaded)
            return; // aborted mid-load by a concurrent unload()
        const { frontmatter, body } = parsed;
        const name = frontmatter.name;
        const description = frontmatter.description;
        const argumentHint = frontmatter["argument-hint"] || "";
        if (!name) {
            this._recordError(file.path, "Missing required frontmatter field 'name'. Add `name: my-skill` " +
                "between the --- markers.");
            return;
        }
        if (!description) {
            this._recordError(file.path, `Missing required frontmatter field 'description' (skill '${name}'). ` +
                "Add `description: One-line summary` so users can see what the skill does in autocomplete.");
            return;
        }
        if (!SKILL_NAME_RE.test(name)) {
            this._recordError(file.path, `Invalid skill name '${name}'. Names must be lowercase, start with ` +
        ...
        if (RESERVED_SKILL_NAMES.has(name)) {
            this._recordError(file.path, `Name '${name}' is reserved by a built-in Gryphon command. ` +
                "Pick a different name to avoid the collision.");
            return;
        }
        if (this.skills.has(name)) {
            const existing = this.skills.get(name);
            this._recordError(file.path, `Name '${name}' is already registered by ${existing.path}. ` +
    ...
    _attachWatchers() {
        ...
        const onDelete = vault.on("delete", (file) => {
            if (this._isSkillFile(file.path))
                this._removeByPath(file.path);
        });
        const onRename = vault.on("rename", async (file, oldPath) => {
            if (this._isSkillFile(oldPath))
                this._removeByPath(oldPath);
            if (file instanceof TFile && this._isSkillFile(file.path)) {
                await this._loadFile(file);
            }
        });
