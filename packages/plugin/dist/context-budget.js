...
function safeStatBytes(p) {
    try {
        ...
        if (st.isFile())
            return st.size;
        if (st.isDirectory())
            return 0; // caller handles dirs explicitly
        return 0;
    }
    catch (_) {
        return 0;
...
function escapeVaultPath(absVaultPath) {
    if (typeof absVaultPath !== "string" || !absVaultPath)
        return "";
...
function summarizeContext({ sources, messages, userInput, windowSize, measuredHistoryTokens, calibrationDelta }) {
    ...
    const chatHistoryTokens = typeof measuredHistoryTokens === "number" && measuredHistoryTokens >= 0
