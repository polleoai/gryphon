...
// under packages/protect/src/. The entry hook files and ipc-client are
// TS sources that must be transpiled; the two shared modules (injection-
// patterns, untrusted-framing) are also TS. All are produced via
// esbuild.transform so the shipped artifacts are plain runnable CJS.
//
// Fallback rule: if a .ts source is absent (partially-migrated tree),
// fall back to the same path with .js extension.
const HOOK_SOURCE_OVERRIDES = {
  "hooks/pretool.js":                   path.join(PROTECT_SRC, "hooks", "pretool.ts"),
  "hooks/posttool.js":                  path.join(PROTECT_SRC, "hooks", "posttool.ts"),
  "hooks/session-start.js":             path.join(PROTECT_SRC, "hooks", "session-start.ts"),
  "hooks/session-end.js":               path.join(PROTECT_SRC, "hooks", "session-end.ts"),
  "hooks/user-prompt.js":               path.join(PROTECT_SRC, "hooks", "user-prompt.ts"),
  "hooks/notification.js":              path.join(PROTECT_SRC, "hooks", "notification.ts"),
...
async function syncHooks() {
  for (const artifactRel of HOOK_FILES) {
    ...
    let src = HOOK_SOURCE_OVERRIDES[artifactRel]
      || path.join(PROTECT_SRC, artifactRel);
    // Fallback: if the configured .ts source is absent (partially-migrated
    // tree), try the same path with .js so a partially-done migration still
    // builds correctly.
    if (src.endsWith(".ts") && !fs.existsSync(src)) {
      const jsFallback = src.replace(/\.ts$/, ".js");
      if (fs.existsSync(jsFallback)) src = jsFallback;
    }
    const dest = artifactRel;   // e.g. "hooks/pretool.js"
    const destDir = path.dirname(dest);
    if (!fs.existsSync(destDir)) fs.mkdirSync(destDir, { recursive: true });
    // .ts source files (hook entry files + shared modules migrated to
    // TypeScript) need to be transpiled to plain JS before copying so the
    // hook scripts (which run under bare Node, no tsx loader) can require
    // them. esbuild.transform strips all TS syntax and emits CJS.
    if (src.endsWith(".ts")) {
...
// Step 3 (dist-vendoring task): esbuild bundles @gryphon/* from their TypeScript
// src/ trees directly, bypassing the package.json `main`/`exports` which now
// point at dist/ for external plain-node consumers.  This preserves the pre-dist
// bundle size (~1352 KB) because esbuild tree-shakes the TS source the same way
// as before; the compiled dist/ trees carry extra tsc overhead (module.exports
// wrappers) that would inflate the bundle by ~120 KB otherwise.
//
// The alias map has no effect on the five deep `../../provider-runtime/src/...`
// file-relative requires in plugin.ts — those bypass the exports map entirely.
const GRYPHON_SRC_ALIASES = {
  "@gryphon/protect":        path.resolve(PROTECT_SRC, "index.ts"),
  "@gryphon/provider-config": path.resolve(REPO_ROOT, "packages", "provider-config", "src", "index.ts"),
...
const config = {
  ...
  format: "cjs",
  target: "es2020",
  // Redirect @gryphon/* to src/index.ts so esbuild bundles from TypeScript
  // sources, not the compiled dist/ trees. dist/ is for external consumers.
  alias: GRYPHON_SRC_ALIASES,
