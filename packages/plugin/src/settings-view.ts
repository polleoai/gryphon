...
function renderGryphonSettings(hostPlugin, containerEl, options) {
  ...
  const rerender = typeof options.onRerender === "function"
    ...
    const updateProviderStatusChip = () => {
      if (!providerStatusEl || typeof providerStatusEl.empty !== "function") return;
    ...
    descToTooltip(
      new Setting(containerEl).setName("Fallback provider"),
    ...
    const cliSetting = descToTooltip(
      ...
      .addButton((btn) =>
        btn
          ...
          .onClick(async () => {
            ...
            const key =
              (hostPlugin.settings.anthropicApiKey || "").trim() ||
      ...
      .addButton((btn) =>
        btn
          ...
          .onClick(async () => {
            btn.setDisabled(true).setButtonText("Testing...");
            const key =
              (hostPlugin.settings.openaiApiKey || "").trim() ||
      ...
      .addButton((btn) =>
        btn
          ...
          .onClick(async () => {
            btn.setDisabled(true).setButtonText("Testing...");
            const key =
              (hostPlugin.settings.googleApiKey || "").trim() ||
      ...
      .addButton((btn) =>
        btn
          ...
          .onClick(async () => {
            ...
            const codexPath =
              (hostPlugin.settings.codexPath || "").trim() ||
              findCodexBinary() ||
              "";
      ...
      .addButton((btn) =>
        btn
          ...
          .onClick(async () => {
            ...
            const geminiPath =
              (hostPlugin.settings.geminiCliPath || "").trim() ||
              findGeminiBinary() ||
              "";
